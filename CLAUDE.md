...
3. **Markdown files are the source of truth, always.** `.knowledge/` IS the database. No persistent index that acts as the source of truth. Ephemeral in-memory caches are fine. On-disk *derived* caches (e.g. FTS index) are allowed only if: rebuildable from markdown on demand, fully disposable (`rm -rf` the cache dir recovers correctness), and never treated as authoritative when they conflict with the markdown.
...
6. **Bias toward a minimal MCP tool surface.** The current four — `mm_search`, `mm_write`, `mm_promote`, `mm_close_loop` — cover today's needs. Prefer extending an existing tool over adding a new one. A fifth tool is allowed when genuinely needed, but requires a DECISIONS.md entry with explicit justification. "Forever" is not the rule; "don't add tools casually" is.
...
- `~/Github/OpenKB` (https://github.com/VectifyAI/OpenKB) — Python CLI. Document→wiki batch ingestion (PDF, Word, MD) using LLM compilation. **Orthogonal domain** (batch docs vs session memory) — NOT a wiring or storage reference. Mine for two ideas only: (1) `openkb lint` health-check pattern → informs `mastermind lint` open loop (contradictions, orphans, stale open-loops, budget drift); (2) `concepts/` cross-document synthesis layer → informs `mm_consolidate` open loop (N live entries → unified insight). Full notes in `.knowledge/references/openkb-llm-compiled-wiki-kb-with-lint-concepts-layers.md`.
...
- **Access tracking uses a sidecar JSON cache** at `<scope-root>/.cache/access.json`. Markdown files are never modified on search. The sidecar is loaded lazily into memory; each `IncrementAccess` call does one in-memory map write plus a small atomic JSON write — negligible overhead at current corpus sizes.
