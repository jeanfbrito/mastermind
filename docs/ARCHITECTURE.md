...
1. **`mm_search(query | queries, scopes?, kinds?, project?, tags?, include_pending?, limit?, expand?)`** — primary read. Stdlib-only keyword matcher with tiered fallback chain (see Retrieval flow section below). Supports single-query mode (`query: "..."`) and batch mode (`queries: [...]`) in one call; exactly one of the two must be provided. Batch mode runs each query through the full tiered pipeline independently and concatenates the per-query result blocks into a single response — avoids N serial tool calls when an agent wants multiple angles. All filters and the limit apply uniformly to every query in a batch. See DECISIONS.md 2026-04-10 entry on the batch schema extension.
...
- `mastermind stop` (2026-04-10) — Claude Code Stop hook. Reads `{session_id, stop_reason, message_count, input_tokens, output_tokens}` on stdin and appends one JSONL line to `~/.knowledge/logs/sessions.jsonl`. Pure telemetry — no LLM calls, no extraction, single-digit millisecond runtime. Turn-count gated: entries with `message_count < 4` are flagged `"short": true` so downstream analytics can filter out trivial clarification turns. Respects `MASTERMIND_NO_AUTO_INIT`. Silent on errors (decode failures, missing home dir, write errors) — a Stop hook must not spam stderr on every turn. Note: Claude Code's Stop hook stdin does NOT expose the assistant's response body, so auto-capturing unresolved open-loops from the response is architecturally impossible — see `.knowledge/hooks/phase-5-experiment-stop-hook-*.md` for the blocker.
